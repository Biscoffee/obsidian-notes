...
- 主队列是串行队列，任务在主线程顺序执行。
- 在主线程调用 dispatch_sync 提交到主队列会导致死锁。
- 在子线程调用 dispatch_sync 提交到主队列不会死锁，因为阻塞线程不同。
- dispatch_async 不阻塞当前线程，因此不会导致死锁。
- 死锁的关键是阻塞线程与目标队列执行线程重合。
...
1. 主队列（Main Queue）是一个串行队列，它里面的任务只会在主线程上被取出执行，而且一次只执行一个。
2. 当前正在执行 syncMain 这段代码的，本身就是主队列上的一个任务——也就是说，主队列正“忙”着跑 syncMain，在 syncMain 没结束之前，主队列不会去取下一个任务。
3. dispatch_sync 的语义是：把 block 提交到目标队列，然后阻塞当前线程，直到这个 block 在目标队列上执行完毕，才返回。

把这三点叠加起来，死锁就一目了然了。

三、为什么换成「其他线程 + 同步执行 + 主队列」就不死锁？
...
- dispatch_sync 投递到当前队列会引发死锁。
- 原理：dispatch_sync 阻塞当前线程等待 block 执行，而 block 在队列尾部，形成循环等待。
- dispatch_async 不阻塞调用线程，因此不会死锁。
...
**来源**: 自述　**confidence**: 1.00

- dispatch_barrier_async 用于并发队列的读写隔离
- 提交后等待之前任务完成，自己独占队列执行
- 执行完后队列才继续接受其他 block
- 常用于实现读写锁模式
...
**来源**: 博客　**confidence**: 1.00

- 函数原型：dispatch_group_notify(group, queue, block)，在组任务完成时异步执行block且不阻塞当前线程。
- 可指定回调队列，如主队列用于UI更新或串行队列保证线程安全。
- 支持多次调用notify，每个block在组归零时触发；若组已空，立即执行。
- 典型用于scatter-gather模式，如并行网络请求后刷新UI或批量下载后合并处理。
...
- Dispatch Semaphore 是 GCD 中的信号量，用于线程同步和并发控制。
- 通过持有计数管理访问，类似收费站栏杆机制。
- 计数小于 0 时线程等待，计数大于等于 0 时减 1 且不等待，允许通过。
...
- Dispatch Semaphore 是 GCD 中的同步原语。
- 三个关键方法：dispatch_semaphore_create、dispatch_semaphore_wait、dispatch_semaphore_signal。
- 用于控制并发访问和线程同步。
- 常配合队列实现资源限制或互斥。
...
**来源**: 未知　**confidence**: 0.95

- dispatch_semaphore_create 是 GCD 中用于创建信号量的函数。
- 该函数接受一个参数，初始化信号量的初始计数（信号的总量）。
- 信号量常用于控制并发访问和同步任务。
