...
不同版本的 objc4 在函数拆分和命名上会有差异。旧版源码里常见 `addUnattachedCategoryForClass`、`remethodizeClass`；新版源码中对已经 realized 的类可能直接调用 `attachCategories`，对未 realized 的类则先放入 unattached category 表，等类 realization 时再统一附加。本文下面保留旧版函数名来讲解，因为它更适合展示“先登记，再附加”的核心思路。
...
编译期
  ↓
Category 被编译成 category_t
  ↓
category_t 指针被放进 Mach-O 的 __objc_catlist 段
  ↓
dyld 加载包含 Category 的 image
  ↓
libobjc 通过 dyld 回调收到 image 加载通知
  ↓
map_images / _read_images / load_categories_nolock 读取 Category 列表
  ↓
通过 cat->cls 找到目标类 Class
  ↓
实例侧内容附加到类对象：
instanceMethods / instanceProperties / protocols
  ↓
类侧内容附加到元类：
classMethods / classProperties / protocols
  ↓
如果目标类已经 realized，直接 attachCategories
如果目标类还没 realized，先放入 unattached category 表，等类 realized 时再附加
  ↓
attachCategories 收集方法列表、属性列表、协议列表
  ↓
attachLists 把 Category 的列表插到原有列表前面
  ↓
按需清空方法缓存，避免继续命中旧 IMP
  ↓
之后 objc_msgSend 查找方法时，Category 的同名方法会优先被找到
```
