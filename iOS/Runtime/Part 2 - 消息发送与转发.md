...
// objc-runtime-new.mm:7061  —— 按方法表种类分发的入口3
...
到了转发阶段，C++ 侧返回的不是某个真正的方法实现，而是 `_objc_msgForward_impcache` 这个转发占位 IMP（第 6 节 :7626 定义、循环里 `imp = forward_imp` 返回）。它最终会把消息送进 `_objc_msgForward(id self, SEL _cmd, ...)` 这条转发入口。汇编侧逐字全文：
