...
- **0.2 messenger 家族**：`objc_msgSend` / `objc_msgSendSuper2` / lookup / forward
- **0.3 方法调用的本质**：`[obj foo]` → `objc_msgSend(obj, sel)`
- **0.4 为什么 objc_msgSend 用汇编写**：性能 / 未知参数 / 尾调用

### 第一部分 · 快速路径（缓存命中）
- **1. cache_t 数据结构**（`objc-runtime-new.h:337`）：先摊开汇编要操作的两张表
  - 1.1 `_bucketsAndMaybeMask`：指针与 mask 的融合
  - 1.2 `bucket_t`：`{SEL, IMP}` 与 ptrauth
  - 1.3 哈希与开放寻址探测（`cache_hash` / `cache_next`）
- **2. 快速路径汇编逐段**（`objc-msg-arm64.s`）：拿着 §1 的结构逐条印证
  - 2.1 取 isa → class（`GetClassFromIsa`，呼应 ISA_MASK）
  - 2.2 CacheLookup：按 SEL 哈希定位 bucket
  - 2.3 命中 `br IMP` / 未命中转入慢速查找
...
- **7. 找到了**：`log_and_fill_cache` 回填缓存，闭环回快速路径（:7560）
- **8. 没找到**：动态方法解析（`resolveMethod_locked` :7528 → `resolveInstanceMethod` :7483）

### 第三部分 · 消息转发
- **9. 转发入口**：`_objc_msgForward_impcache`（:7674 / 汇编 :788）
- **10. 转发三部曲**
  - 10.1 快速转发 `forwardingTargetForSelector:`（换 receiver 重发）
  - 10.2 完整转发 `methodSignatureForSelector:` + `forwardInvocation:`
  - 10.3 兜底崩溃 `doesNotRecognizeSelector:`（`NSObject.mm:2578`）
- **11. 边界澄清**：objc4 只到 `_objc_msgForward`，三部曲调度在 CoreFoundation `___forwarding___`（闭源）
- **12. 其实发生了两次动态方法决议**：正常消息查找一次，完整转发签名路径里还可能再来一次

### 第四部分 · 实战与收尾
- **13. LLDB 实战**：cache 从空到有 / 命中对比 / 慢速断点 / 转发链逐站拦截
...
>
> 新旧对照说明：文中的「旧→新对照」框，旧版取自本地 **objc4-818.2**（可调试版）真实源码、新版为 **951.7**，均标注精确 `file:line`。注意：缓存早期那次著名的大改造（`_buckets`/`_mask`/`_occupied` 三个分离字段 → 融合成 `_bucketsAndMaybeMask`）发生在 **781 之前**，已用本地 **objc4-756.2**（融合前末代，真实源码 `objc-runtime-new.h:82`）补做对照（见 1.1 末）；其余对照的旧版多取自 818.2，818.2 与 951.7 同属「融合字段后」时代，是这之后的增量演进。所有旧版代码均据真实源码。
...
### 0.4 为什么 objc_msgSend 用汇编写

三个原因：
- 调用频率极高，手写汇编优化性能，榨干每一周期；
- 在**不知道目标方法参数个数/类型**的情况下原样透传所有寄存器；
- 命中后用**尾调用**（`br`）直接跳进 IMP，自己不留栈帧。与下面第 2.3 节的命中分支 `br x17`相对应
...
#   define CACHE_T_HAS_FLAGS 1
    ...
    static constexpr uintptr_t maskZeroBits = 4;	//紧邻 mask 的 4 位必须为 0，
							// 让 msgSend 一条指令算出 mask<<4（见 2.2 的 LSR #(48-(1+PTRSHIFT))）
...
> ```
>
> - A11 及更早（无指针认证）走这支：hash 参数 `<< 48`，bit 63..53 放 hash_mask、52..48 放 hash_shift——和 PAC 版（63..60 mask 位数、59..55 hash_shift、`0x7fff >> mask_bits`）的打包方式不同。这与 §2.2 汇编 ② 里"只留 ARM64e 真机分支"的口径一致。


而这套「融合字」的拆/装，C++ 侧有对应实现——和 2.2 的汇编一一镜像：
...
`setBucketsAndMask`（`(mask<<48)|buckets`）= 汇编看到的那个融合字是怎么写进去的；`mask()`（`>>48`）/`buckets()`（`& bucketsMask`）= 汇编 `lsr`/`and` 在 C++ 里的等价拆解。换言之第 2.2 节那几条指令，就是把这三个 C++ 函数手写进了汇编快速路径。
...
- **为什么在 1.1 的字段里看不到 `bucket_t`**：`_bucketsAndMaybeMask` 声明成 `uintptr_t`（整数）而非 `bucket_t *`；1.1 只给了 `bucketsMask`/`maskShift` 等「解码常量」（钥匙），真正执行 `& 掩码 + 强转` 的动作在这里的 `buckets()` 里。所以在 1.1 那段按 `bucket_t` 去搜，搜不到是必然的——钥匙在抽屉里，开门的手在这一节。
...
```
三字段 变更为 一字段：`_mask` 不再单独存，挤进 `_bucketsAndMaybeMask` 的高 16 位（省一次内存读取，且第 2.2 节那条「一条 `ldr` 同时取出 mask 和 buckets」正因此成立）；新增的 union 是给 dyld 共享缓存「预优化缓存」让位。`_occupied` 保留。

#### 1.2 `bucket_t`：`{IMP, SEL}` 与 ptrauth（:214）
...
arm64 上 IMP 在前、SEL 在后，所以 2.2 的 `ldp p17, p9`（先 imp 后 sel）顺序与此一致；命中时用 `modifierForSEL`（`buckets_base ^ sel ^ cls`）重算修饰子解签，正是 2.3 实测的两条 `eor`。
...
#### 1.3 哈希与开放寻址探测（`cache_hash` :307 / `cache_next` :241）
...
§1 把两张表（`cache_t` / `bucket_t`）和哈希探测讲清楚了，下面逐条读汇编时，每条指令都能对应回去：`ldr [x0]` 取的是 §1.1 的 isa、`[x16, #CACHE]` 取的是 §1.1 的融合字、`ldp` 拆的是 §1.2 的 `bucket_t`、命中时两条 `eor` 重算的是 §1.2 的 `modifierForSEL`。
...
;; objc-msg-arm64.s:587  —— _objc_msgSend 入口（全文）
	...
	GetClassFromIsa_p16 p14, 1, x0	// p16 = class
					//抹标志位 + ptrauth 解签 → 真正的 Class（见 2.1）
LGetIsaDone:
	// calls imp or objc_msgSend_uncached
	CacheLookup NORMAL, _objc_msgSend, __objc_msgSend_uncached
					//查缓存：命中尾跳 IMP，未命中跳 uncached（见 2.2/2.3）
...
#### 2.0 先看类对象布局：`#CACHE` / `#SUPERCLASS` 偏移哪来的

上面入口里 `[x0]` 取 isa、后面 2.2 的 `[x16, #CACHE]`、第 6 节的 `[x16, #SUPERCLASS]`，这些偏移都来自 `objc_class` 的内存布局：

```objc
// objc-runtime-new.h:2635 —— 类对象本身就是一个 objc_class
struct objc_class : objc_object {
    // Class ISA;          //偏移 0：继承自 objc_object（2.1 节 ldr [x0] 取的就是它）
...
#define BUCKET_SIZE      (2 * __SIZEOF_POINTER__)  // = 16  → 2.2 的 ldp …, #-BUCKET_SIZE
```

对照 2.2 的 LLDB 反汇编 `ldr x10, [x16, #0x10]`：`0x10` = 16 = `#CACHE`，取的正是 `cache` 字段（即融合字 `_bucketsAndMaybeMask`）；`ldp …, #-0x10` 里的 `0x10` 就是 `BUCKET_SIZE`（每个 bucket 16 字节）。

#### 2.1 取 isa → class（`GetClassFromIsa_p16`，:115，呼应 Part 1 的 ISA_MASK）
...
#### 2.2 CacheLookup：按 SEL 哈希定位 bucket（:336）
...
1. **一条 `ldr` 取出融合字**：`[x16, #CACHE]` 一次性读出 §1.1 的 `_bucketsAndMaybeMask`——高 16 位是 mask、低 48 位是 buckets 指针。HIGH_16 的精明之处是**不急着拆**：下一条算哈希时直接用 `p11, LSR #48` 顺手把 mask 移出来用，省掉一条独立的拆字指令（正是 §1.1 `maskZeroBits` 那条注释说的「一条指令顺手算出 `mask`」）。
2. **算下标 = §1.3 的 `cache_hash`**：`SEL_HASH_SHIFT_XOR` 版先 `sel ^ (sel>>7)` 把高位搅进低位、**减少哈希冲突**，再 `& mask`。`& mask` 之所以等价于「对桶数取模」，是因为桶数恒为 2 的幂，结果必落在 `[0, mask]`。（非 XOR 版直接 `_cmd & mask`，因为 SEL 本质是字符串地址、低位已够随机。）
...
- **命中**（`sel == _cmd`）：跳 `CacheHit`，按 `\Mode` 分流——NORMAL 直接 `br` 跳进 IMP、GETIMP 把 IMP 放 x0 返回、LOOKUP 返回 IMP 不跳（三模式详见 **§2.3**）。
- **撞空槽**（`sel == 0`）：说明这个 selector 从没缓存过，`cbz` 直接跳 `_objc_msgSend_uncached` 走**慢速查找**（第二部分），找到后再回填缓存。
- **都不是**：`ldp ..., #-BUCKET_SIZE` 已让指针**向低地址挪一格**——这正是 §1.3 `cache_next`（arm64 `i ? i-1 : mask`）的汇编落地。只要还没退到表头（`bucket >= buckets`）就回 `1:` 继续。注意 ObjC 缓存的探测方向是==**从高地址往低地址**==。
...
;; 寄存器约定：x16=isa/cls  x1=_cmd(SEL)  x10=buckets基址  x11=mask  x13=当前bucket  x9=槽内SEL  x17=槽内IMP
  ...
  <+44>: eor  x12, x1, x1, lsr #7       ; sel ^ (sel>>7)  —— 对应 §1.3 cache_hash 的 SEL_HASH_SHIFT_XOR
...
;; ---------- CacheHit：命中 ----------
  ...
  <+72>: eor  x10, x10, x16             ; x10 = buckets ^ sel ^ cls  ← modifierForSEL（3 项！呼应 1.2）
...
命中:  imp = auth(imp, buckets ^ sel ^ cls);  br imp   // 解签后尾跳，不留栈帧（§2.3 / §1.2）
未命中: b _objc_msgSend_uncached                        // 转入 lookUpImpOrForward（第二部分）
```


#### 2.3 命中 `br IMP` / 未命中转入慢速查找（`CacheHit` :316）
...
> **注**：写入路径（`cache_t::insert`）并不在快速路径执行过程中触发——快速路径只读 cache。`insert` 的实际触发点是第二部分第 7 节：慢速查找成功后，`log_and_fill_cache` 回填缓存时才调用它。本节放在第一部分，是因为 cache 的读/写/结构三者放在一起对照阅读更方便，并非表示写入属于快速路径的执行流。
...
//reallocate 内部只分配一张新的空 bucket_t[] 数组，旧表整张丢弃、不迁移——源码注释写得很直白：// Cache's old contents are not propagated.（详见 §3.2）。旧条目不会被重新哈希搬过去，而是靠之后的 miss 慢速查找重新 insert 懒填回来。扩容后 buckets() 返回的 base 地址变了，旧条目里 IMP 的 ptrauth 签名都含旧 base，本就随旧表一起被丢弃；重填时由 insert 用新 base 重新签名落位。这正是 modifierForSEL 把 base 纳入 discriminator 的好处：换表即让旧签名自然失效，无需任何额外的失效机制。
...
```
新（951.7）把「判装填 + 扩容 + 探测落位」**合并进一个 `cache_t::insert()`**（见 3.1 全文），`reallocate` 还多了 `freeOld` 参数管旧表回收。

一处**没变**的设计（756.2 `:476` 与 951.7 `:814` 注释一字不差）：`// Cache's old contents are not propagated.`——扩容即丢弃旧表、不迁移，从那时起就是如此，并非新近退化。

#### 3.3 装填因子与留空策略

`insert` 里 `cache_fill_ratio(capacity)`（默认 3/4）就是装填因子上限；2.2 探测循环靠 `cbz p9` 撞空槽判定 miss——3/4 装填上限通常保证有空槽；但启用 `CACHE_ALLOW_FULL_UTILIZATION` 时小容量表（`capacity <= FULL_UTILIZATION_CACHE_SIZE`）可达 100%，此时 wrap-around 靠「绕回出发点」而非「撞空槽」终止（正是 §2.2 引用的那条注释所解释的）。
...
### 4. lookUpImpOrForward：加锁 + realize / +initialize（:7672）
...
```
新（951.7，`:7110`）—— `switch (list->listKind())` 三种 Kind（**新增 `bigSigned`**），且共享缓存里改成按相对偏移 `compare(keyOffset, ...)` 直接比较，不再先解析出 SEL（全文见 5.2 上方）。

两点实质变化：① 方法名指针在 arm64e 上可被签名，于是多出 `bigSigned` 这一 Kind；② 共享缓存场景用 `key - sharedCacheRelativeMethodBase()` 得到的偏移直接比，省掉「解引用取 SEL」一步。

### 6. 沿 superclass 链逐层上溯（每层先查父类 cache）

逻辑全在第 4 节 `lookUpImpOrForward` 的 `for` 主循环里
...
//这是相对较新的机制，给系统级监控工具用的。`msgSendCacheMissHook` 是一个全局可设置的回调点——任何挂上来的钩子都会在每次缓存未命中并最终命中实现时被调用一次。
    ...
    cls->cache.insert(sel, imp, receiver);		// 第 3 节 insert，落进本类缓存
}
```

回填后下一次同样的消息就在第 2 节快速路径命中。**LLDB 实测**：`[d bark]` 第一次停在 `lookUpImpOrForward`，第二次**没有再停**（命中直接 `brab`），两行 `woof` 连续打印、进程干净退出：
...
### 8. 没找到：动态方法解析 && 决议（`resolveMethod_locked` :7528 → `resolveInstanceMethod` :7483）
...
### 9. 转发入口：`_objc_msgForward_impcache`（C++ 侧 :7674 / 汇编 :788）

> 易混提示：汇编 `:779` 只是注释块开头（`* id _objc_msgForward(...)`），真正的 `STATIC_ENTRY __objc_msgForward_impcache` 在 **:788**。

到了转发阶段，C++ 侧返回的不是某个真正的方法实现，而是 `_objc_msgForward_impcache` 这个转发占位 IMP（第 4 节 :7674 定义、循环里 `imp = forward_imp` 返回）。它最终会把消息送进 `_objc_msgForward(id self, SEL _cmd, ...)` 这条转发入口。汇编侧逐字全文：
...
********************************************************************/
	...
	//尾跳进 _objc_forward_handler；带 CF 的进程里它被换成 CF 的转发实现（见第 11 节）
...
三步都由 CoreFoundation 的 `___forwarding___` 串起来调用（第 11 节）。下面给出每一步**实测命中的真实实现与调用栈**。
...
总结：第 12 节真正要抓住的是 “同一个 selector，可能被两条路径各查一次”。第一次是原始消息发送失败后，`objc_msgSend` 慢速路径给类一次补方法的机会；第二次是完整转发为了构造 `NSMethodSignature`，在默认签名探测路径里通过 `class_getInstanceMethod` 再次触发带 resolver 的方法查找。前者决定消息能不能回到正常派发，后者决定完整转发有没有资格继续构造 `NSInvocation`。所以调试时看到 resolver 出现两次，不要急着判断是递归或重复发送；先看调用栈里有没有 `methodSignatureForSelector:` / `class_getInstanceMethod` 这一截，有的话，它就是签名探测带出来的第二轮动态方法决议。
...
- **cache 从空到有 / 命中对比**：见第 7 节——`[d bark]` 第一次进 `lookUpImpOrForward`，第二次不再进。
- **慢速断点**：`p (char*)sel_getName($x1)`、`po (Class)$x2` 读出 receiver/SEL/cls。
- **转发链逐站拦截**：见第 8–10 节真实调用栈，基础失败路径为：
